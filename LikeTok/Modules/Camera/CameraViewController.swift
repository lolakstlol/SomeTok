...
class CameraViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        toggleFlashButton.addBlurEffect(style: .regular, cornerRadius: 22, padding: 0)
        toggleCameraButton.addBlurEffect(style: .regular, cornerRadius: 22, padding: 0)
    ...
    @IBAction func infoDidTap(_ sender: Any) {
        let infoToast = AboutPersonalPostAssembler.createModule()
        infoToast.modalPresentationStyle = .overCurrentContext
        present(infoToast, animated: true, completion: nil)
    }
    ...
    func uploadVideoFlow(video: Data, preview: UIImage) {
        ...
        case .ad:
            ...
            let vc = VideoUploadAdvertismentFinalAssembler.createModule(preview: data, video: video)
...
extension UIButton {
    func addBlurEffect(style: UIBlurEffect.Style = .regular, cornerRadius: CGFloat = 0, padding: CGFloat = 0) {
        ...
        blurView.frame = bounds.inset(by: UIEdgeInsets(top: padding, left: padding, bottom: padding, right: padding))
